...
class QuestionQueries(graphene.ObjectType):
    get_questions_modified_after = graphene.Field(
        SelectOutputType,
        filter = QuestionModifiedFilterInput(required=True)
    )

    get_questionset_modified_after = graphene.Field(
        SelectOutputType,
        filter = QuestionSetModifiedFilterInput(required=True)
    )

    get_questionsetbelonging_modified_after = graphene.Field(
        SelectOutputType,
        filter = QuestionSetBelongingModifiedFilterInput(required=True)
    )

    @staticmethod
    def resolve_get_questions_modified_after(self, info, filter):
        try:
    ...
    @staticmethod
    def resolve_get_questionset_modified_after(self, info, filter):
        try:
    ...
    @staticmethod
    def resolve_get_questionsetbelonging_modified_after(self, info, filter):
        try:
