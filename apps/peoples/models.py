...
class People(AbstractBaseUser, PermissionsMixin, TenantAwareModel):
    ...
    loginid       = models.CharField(_("loginid"), max_length=50, unique=True)
    ...
    reportto      = models.ForeignKey("self", null=True, blank=True, on_delete=models.RESTRICT, related_name='people_reportto')
    deviceid      = models.CharField(_("deviceid"), max_length=50, default='-1')
    cuser         = models.ForeignKey("self", null=True, blank=True, on_delete=models.RESTRICT, related_name='created_by')
    muser         = models.ForeignKey("self", null=True, blank=True, on_delete=models.RESTRICT, related_name='modified_by')
    cdtz          = models.DateTimeField(_("cdtz"), auto_now=False, auto_now_add=True)
...
class BaseModel(models.Model):
    ...
    cdtz  = models.DateTimeField(_('cdtz'), auto_now_add = True)
    mdtz  = models.DateTimeField(_('mdtz'), auto_now = True)
...
class Pgroup(BaseModel, TenantAwareModel):
    groupid    = models.BigIntegerField(_('groupid'), primary_key=True)
...
class Pgbelonging(BaseModel, TenantAwareModel):
    pgbid       = models.BigIntegerField(primary_key=True)
...
class Capability(BaseModel, TenantAwareModel):
    CFOR_CHOICES = [('W', 'WEB'), ('P', 'PORTLET'), ('R', 'REPORT'), ('M', 'MOB')]
    
    capsid      = models.BigAutoField(primary_key=True)
    caps        = models.CharField(_('caps'), max_length=50)
    includes    = models.CharField(_('includes'), max_length=1000, default = None, blank=True, null=True)
    description = models.CharField(_('description'), max_length=200, default = "")
    enable      = models.BooleanField(_('enable'), default = True)
    ...
    class Meta(BaseModel.Meta):
        ...
        get_latest_by       = ["mdtz",'cdtz']
        constraints = [
        models.UniqueConstraint(fields=['caps', 'cfor'], name="capability_caps_cfor_uk"),]
...
class PeopleEventlog(BaseModel, TenantAwareModel):
    pelogid         = models.BigIntegerField(_('pelogid'), primary_key=True)
