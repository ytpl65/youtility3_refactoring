...
def process_jobneed(job, params, crontype, people, pdtz, jobstatus, jobtype):
    ...
    is_parent = crontype in (am.Job.Identifier.INTERNALTOUR.value, am.Job.Identifier.EXTERNALTOUR.value)
...
def update_jobneed_expiry(job, jobneed, crontype, pdtz, people, jobstatus, jobtype):
    if isinstance(jobneed, am.Jobneed):
        ...
        if crontype in (am.Job.Identifier.INTERNALTOUR.value, am.Job.Identifier.EXTERNALTOUR.value):
            ...
            if edtz is not None:
                num_updated = am.Jobneed.objects.filter(id = jobneed.id).update(expirydatetime = edtz)
                if num_updated <= 0:
                    raise ValueError
...
def insert_into_jn_for_parent(job, params):
    ...
    jobneed = am.Jobneed.objects.create(
        **jobneed_defaults,
        job_id=job['id'],
...
def insert_update_jobneeddetails(jobneed_id, job, is_parent=False):
    ...
    try:
        am.JobneedDetails.objects.filter(jobneed_id=jobneed_id).delete()
    ...
    try:
        if is_parent:
            ...
            question_set = am.QuestionSetBelonging.objects.select_related('question').filter(qset_id=job['qset_id']).order_by('seqno').values_list(named=True)
...
def retrieve_jobs(job):
    ...
    """
    return am.Job.objects.annotate(
...
def update_params(job, job_data, params, index):
    ...
    """
    asset = am.Asset.objects.get(id=job_data['asset_id'])
