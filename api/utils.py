...
def diplay_input(input):
    log.info('Displaying data recived from client:\n')
    ic(input)
    print()
...
def save_parent_childs(sz, jn_parent_serializer, child, msg):
            ...
            if child_serializer.is_valid():
                ...
                for dtl in details:
                    ...
                    if ch_detail_serializer.is_valid():
                        ...
                        traceback, msg, rc = str(ch_detail_serializer.errors), msg.INSERT_FAILED, 1
                allsaved+=1
            else:
                log.error(child_serializer.errors)
                traceback, msg, rc = str(child_serializer.errors), msg.INSERT_FAILED, 1
        if allsaved == len(child):
            msg, returnid = msg.INSERT_SUCCESS, parent.id
    else:
        log.error(jn_parent_serializer.errors)
        traceback, msg, rc = str(jn_parent_serializer.errors), msg.INSERT_FAILED, 1
...
def perform_reportmutation(file):
    ...
    instance = None
    try:
        data, _ = apiutils.get_json_data(file)
        if data:
            ...
            try:
                with transaction.atomic(using=utils.get_current_db_name()):
                    if child and len(child) > 0 and parent:
                        ...
                        rc, returnid, traceback, msg = apiutils.save_parent_childs(sz, jn_parent_serializer, child, Messages)
                    else:
...
def perform_tasktourupdate(file, request):
    ...
    try:
        data = apiutils.get_json_data(file)
        ic(data)
