...
function calculateLatLngPoints(response, data, routeFreq){
    ...
    optimizedPoints[0]['distance'] = optimizedPoints[0]['duration'] = optimizedPoints[0]['expirytime'] = 0
...
function calculateDistanceDuration(response, data){
    ...
    for(var i= 0; i < legs.length; i++){
        let l=[]
        optimizedPoints[i+1]['distance'] = parseFloat(legs[i]["distance"]["value"] / 1000).toFixed(2);
...
function d2DrawMarker(row, idx, routeFreq=1, end=false, remain=false, no=null, left=null) {
    ...
    google.maps.event.addListener(markerC, 'click', function () {
        ...
        var infoWindowHtml= "<span style= 'font-weight: bold;font-size:16px'>" + row['buname'] + "</span>";
...
function ManageFrequenciedRoutes(data, routeFreq){
    ...
    for(var f= 0; f < routeFreq; f++) {
        for(var i= 0; i < optimizedPoints.length - 1; i++){
            ...
            if( f > 0 &&  i == 0) {
                ...
                fPoint[fCnt]['distance'] = optimizedPoints[optimizedPoints.length - 1]['distance'];
                fPoint[fCnt]['duration'] = optimizedPoints[optimizedPoints.length - 1]['duration'];
                fPoint[fCnt]['expirytime'] = optimizedPoints[optimizedPoints.length - 1]['expirytime'];
            } else {
    ...
    for(var i= 0; i < optimizedPoints.length; i++) {
        calDateTime= moment(calDateTime).add(optimizedPoints[i]['expirytime'], 'minutes').format("YYYY-MM-DD HH:mm:ss");
    }
    return fPoint
}
...
function getSitesData(params){
    ...
    for(let i = 0; i<sitesData.length; i++){
        ...
        obj['jobname'] = `${$("#id_jobname").val()} - ${i} - ${sitesData['buname']}`
